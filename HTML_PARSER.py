...
def PARSER(x):
    try:
        http = httplib2.Http(
            disable_ssl_certificate_validation=True)  # А это хттплиб2, ага. Он работает по-другому. Ваш К.О. p.s. Это для http1, http2 не поддерживает httplib2, для http2 нужен hyper, см. поиск гугла.
        headers = {
