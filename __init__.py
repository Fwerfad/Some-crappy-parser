        ...
        try:
            ...
            if soup != None:
                ...
                if meta != "404 Not Found" and text != "":  # Если мета 404 страницы или нету текста - мимо
                    ...
                    if not path.exists(
                            path.abspath("Output" + "\\" + x + "\\" + str(H))):  # Место хранения уже готовой информции
                        makedirs(path.abspath("Output" + "\\" + x + "\\" + str(H)))
                    with open(path.abspath("Output" + "\\" + x) + '\\' + str(H) + "\\" + "Body" + ".html",
                              "wb") as Sf:  # Сама страница.
                        Sf.write(soup.prettify('utf-8'))
                        Sf.close()
                    with open(path.abspath("Output" + "\\" + x) + '\\' + str(H) + "\\" + "Text" + ".txt",
                              "w") as Sf:  # Текст страницы
                        Sf.write(text)
                        Sf.close()
                    with open(path.abspath("Output" + "\\" + x) + '\\' + str(H) + "\\" + "Meta" + ".txt",
                              "w") as Sf:  # Мета страницы
                        Sf.write(meta)
                        Sf.close()
                    with open(path.abspath("Output" + "\\" + x) + '\\' + str(H) + "\\" + "Content" + ".html",
                              "w") as Sf:  # Текст страницы с тегами
                        Sf.write(Content)
                        Sf.close()
                    with open(path.abspath("Output" + "\\" + x) + '\\' + str(H) + "\\" + "Urls" + ".txt",
                              "w") as Sf:  # Все ссылки с ключевого слова
                        Sf.write(Content)
                        Sf.close()
                    D += text + "\n" + "\n"
                    time.sleep(0.5)
